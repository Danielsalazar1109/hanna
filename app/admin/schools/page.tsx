...
export default function AdminSchoolsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(isErrorResponse(data) ? data.error : "Failed to load schools.");
      }

      const loaded = (data as { items?: unknown }).items;
      setItems(Array.isArray(loaded) ? (loaded as SchoolItem[]) : []);
    } catch (e) {
      setError(String(e));
    } finally {
      setLoading(false);
  ...
  async function patch(id: string, patchBody: Partial<SchoolItem>) {
    setError(null);
    ...
    <div>
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Manage schools shown to students when booking.
        </p>
      </div>
